...
size(600,400);
y=height;
fontcredit = loadFont ("CourierNewPS-BoldItalicMT-48.vlw");
fontsub = loadFont ("CourierNewPS-BoldMT-48.vlw");
...
void draw(){
background(200);
if(aventura == false && creditos == false){inicio = true; 
if (inicio){
  t=0;
  aventura = false; creditos = false;
...
if (aventura){
  ...
  if(t >= 2 && t < 3 ){image(flujoBC[2],0,0,width,height-70);text("Creció acompañado de su amigo el raton, tranquilo con su nueva familia......\nFIN",5,350);}if(t >= 3){creditos = true; t= 0; flujo[13] = false;}
  ...
  textFont(fontsub,10);
  text("Precione RIGHT para continuar o LEFT para volver a atras",0,20);  }//FIN DE IF-AVENTURA
if (creditos){
  y-=5;
  background(250);
  image(image[0],0,y,200,200);
  image(image[1],0,(y + 950),200,200); 
  image(image[2],0,(y + 1300),200,200);
  textFont(fontsub,20);
  text("Escrito por",0,(y + 300));
  text("Canciones y particulas de",0,(y + 400));
  text("Con las voces de",0,(y + 500));
  text("Dirigido por",0,(y+1150));
  textFont(fontsub,15);
  text("Inspirado en la historia de Hans Christian Andersen",0,(y + 280));
  textFont(fontcredit,50);//nombres
  text("EL PATITO FEO",0,(y + 250));
  text("Sue Radley & Martin Gates",0,(y + 350));//Escrito por
  text("Christian Caswell",0,(y+450)); //Canciones y particulas de
  //Con las voces de
  text("Paul Pantig",0,(y + 550));
  text("Imelda Staunton",0,(y + 600));
  text("Hugh Laurie",0,(y + 650));
  text("Alison Steadman",0,(y + 700));
  text("Peter Bayliss",0,(y + 750));
  text("Gary Martin",0,(y + 800));
  text("Maria Darling",0,(y + 850));
  text("Sally Kinghorn",0,(y + 900));
  text("Mark Lockyer",0,(y + 950));
  //Dirigido por
  text("David Elvin",0,(y + 1200)); 
  text("Marin Gates",0,(y + 1250));
  if( y == -1500){creditos =false; aventura = false;}
}//FIN DE IF-CREDITOS
}//FIN DEL DRAW
void keyPressed() {
